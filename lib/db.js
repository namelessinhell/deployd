...
function getConnection(db) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    mongodb.MongoClient.connect(db.connectionString, db.connectionOptions).then(function (database) {
      db.Db = database;
      resolve(database);
    }).catch((e)=> {
      error(e);
...
function collection(store, fn) {
  ...
  return getConnection(db).then(function (mdb) {
    return new Promise(function(resolve, reject) {
        var funcCollection = null;
        if(mdb.hasOwnProperty("collection")){
            funcCollection = mdb.collection;
        }else{
            funcCollection = mdb.db(db.options.name);
        }

        try{

          var collection = funcCollection.collection(store.namespace);
          if(!collection) {
            error(new Error('Unable to get ' + store.namespace + ' collection'));
            process.exit(1);
          }
...
Store.prototype.count = function(query, fn) {
  ...
  collection(this, function (err, col) {
    ...
    console.log(options);
    col.countDocuments(query, fields, options).then(function(count) {
      fn(null, count);
    }).catch((err) => {
      if (err) return fn(err);
    });
...
Store.prototype.remove = function (query, fn) {
  ...
  collection(this, function (err, col) {
    ...
    col.deleteMany(query, function (error, r) {
      fn(error, r ? { count: r.deletedCount } : null);
    });
  });
};
		...
		if(typeof query[x]=="object"&&query[x]!=null) {
				...
				if(query[x].$gt)
					if(query[x].$gt.indexOf("new Date")!=-1){
                        ...
                        query[x].$gt.setHours(query[x].$gt.getHours() + myHours);
                    }

				if(query[x].$gte)
					if(query[x].$gte.indexOf("new Date")!=-1){
						query[x].$gte = eval(query[x].$gte);
                        query[x].$gte.setHours(query[x].$gte.getHours() + myHours);
                    }

				if(query[x].$lt)
					if(query[x].$lt.indexOf("new Date")!=-1){
						query[x].$lt = eval(query[x].$lt);
                        query[x].$lt.setHours(query[x].$lt.getHours() + myHours);
                    }

				if(query[x].$lte)
					if(query[x].$lte.indexOf("new Date")!=-1){
						query[x].$lte = eval(query[x].$lte);
                        query[x].$lte.setHours(query[x].$lte.getHours() + myHours);
                    }
		}
	}
